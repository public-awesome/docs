...
{% hint style="info" %}
The CAR format for metadata requires files to not have extensions. So either remove the .json extension from each file, or switch to branch `nft_storage` which removes the file extensions.
{% endhint %}
...
Update your `config.js` with this `baseURI` in the following format: `ipfs://[CID]/metadata`. So for this example, it would be:

```
ipfs://bafybeieousyzgrlt6xyn75dkxrmhy5ynje2kmft3qmavgihylenn4slycm/metadata
```
